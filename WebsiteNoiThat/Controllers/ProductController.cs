...
namespace WebsiteNoiThat.Controllers
{
	public class ProductController : Controller
	{
		...
		// Thêm tham số int? page để nhận biết đang ở trang mấy
		public ActionResult CategoryShow(int? cateId, int? page)
		{
			...
			var category = db.Categories.Find(cateId);
			ViewBag.CategoryName = category != null ? category.Name : "Danh mục";
			ViewBag.IdCategory = cateId;
		...
		public async Task<ActionResult> DetailProduct(int? Id)
		{
			...
			var model = productDao.DetailsProduct(Id.Value);
			...
			// Lấy danh sách ảnh chi tiết từ bảng AnhChiTiet
			var imageList = productDao.ListImagesByProductId(Id.Value);
			...
			try
			{
				...
				}))
				{
					...
					for (int i = 0; i < maxRetry; i++)
					{
						try
						{
							...
							if (response.IsSuccessStatusCode)
							{
								...
								if (!recommendProducts.Any())
									recommendProducts = db.Products.OrderByDescending(p => p.Quantity).Take(4).ToList();
						...
						{
							if (i == maxRetry - 1)
								return db.Products.OrderByDescending(p => p.Quantity).Take(4).ToList();
